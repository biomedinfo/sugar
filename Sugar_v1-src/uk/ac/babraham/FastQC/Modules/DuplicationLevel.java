...
//		System.out.println("Non unique percentage is "+duplicatePercentTotal);
	...
	public JPanel getResultsPanel() {
		...
		String title = "Sequence Duplication Level >= "+df.format(duplicatePercentTotal)+"%";
		ChartPanel newPanel = FreeChartFactory.createDuplicationLevelGraph(counts, labels, title);
...
//		return new LineGraph(new double [][] {counts}, 0d, maxCount, "Sequence Duplication Level",new String [] {"%Duplicate relative to unique"}, labels, "Sequence Duplication Level >= "+df.format(duplicatePercentTotal)+"%");
		JPanel oldPanel = new LineGraph(new double [][] {counts}, 0d, maxCount, "Sequence Duplication Level",new String [] {"%Duplicate relative to unique"}, labels, "Sequence Duplication Level >= "+df.format(duplicatePercentTotal)+"%");
	...
	public void makeReport(HTMLReportArchive report) throws IOException {
		...
		zip.putNextEntry(new ZipEntry(report.folderName()+"/Images/duplication_levels_ext.png"));
		
		b = new BufferedImage(800,600,BufferedImage.TYPE_INT_RGB);
		g = b.getGraphics();
		
		String title = "Sequence Duplication Level >= "+df.format(duplicatePercentTotal)+"%";
		ChartPanel chartPanel = FreeChartFactory.createDuplicationLevelGraph(counts, labels, title);
		...
		sb.append("<p><img class=\"indented\" src=\"Images/duplication_levels_ext.png\" alt=\"Duplication level graph - extended\"></p>\n");
