...
import uk.ac.babraham.FastQC.Graphs.FreeChartFactory;
...
public class PerSequenceQualityScores implements QCModule {
	...
	public JPanel getResultsPanel() {
		...
		ChartPanel newPanel = FreeChartFactory.createSequenceQualityScoresGraph(xCategories, qualityDistribution);
		
		JPanel oldPanel = new LineGraph(new double [][] {qualityDistribution}, 0d, maxCount, "Mean Sequence Quality (Phred Score)",new String [] {"Average Quality per read"}, xCategories, "Quality score distribution over all sequences");
		JPanel result = new JPanel(new BorderLayout());
		JSplitPane splitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, oldPanel, newPanel);
	...
	public void makeReport(HTMLReportArchive report) throws IOException {
		...
		zip.putNextEntry(new ZipEntry(report.folderName()+"/Images/per_sequence_quality_ext.png"));
		
		b = new BufferedImage(1200,600,BufferedImage.TYPE_INT_RGB);
		g = b.getGraphics();
		
		// write the additional chart - using jFreeChart		
		ChartPanel chartPanel = FreeChartFactory.createSequenceQualityScoresGraph(xCategories, qualityDistribution);
		...
		// add additional chart to html page 
		sb.append("<p><img class=\"indented\" src=\"Images/per_sequence_quality_ext.png\" alt=\"Per Sequence quality graph - extended\"></p>\n");
