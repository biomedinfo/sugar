...
public class QualityBoxPlot extends JPanel {
	...
	private static final int DEFAULT_QUALITY_THRESHOLD = 30;
	private static final Color QUALITY_THRESHOLD_LINE_COLOR = Color.ORANGE;
	private static final int QUALITY_THRESHOLD_LINE_THICKNESS = 3;
	...
	public void paint (Graphics g) {
		...
		// draw lower quality threshold
		int qualityThreshold = getQualityThreshold();
		Color color = g.getColor();
		g.setColor(QUALITY_THRESHOLD_LINE_COLOR);
		g.fillRect(xOffset, getY(qualityThreshold) - QUALITY_THRESHOLD_LINE_THICKNESS / 2, getWidth() - 10 - xOffset, QUALITY_THRESHOLD_LINE_THICKNESS);
		g.setColor(color);
...
//			System.out.println("For base "+i+" Yvalues are BoxBottom="+boxBottomY+" boxTop="+boxTopY+" whiskerBottom="+lowerWhiskerY+" whiskerTop="+upperWhiskerY+" median="+medianY);
	...
	private static int getQualityThreshold(){
		int result = DEFAULT_QUALITY_THRESHOLD;
		String qcOption = System.getProperty("fastqc.qc_threshold_line");
		if (qcOption != null) {
			try {
				result = Integer.parseInt(qcOption);
			} catch (NumberFormatException e) {
				System.err.println("Quality check threshold '"+System.getProperty("fastqc.qc_threshold_line")+" wasn't a number");
			}
		}
		return result;
	}
