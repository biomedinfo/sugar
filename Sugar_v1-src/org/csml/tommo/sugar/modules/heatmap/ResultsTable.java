...
public class ResultsTable extends JTable implements PropertyChangeListener {
	 ...
	 * @param arg0
	 */
	public ResultsTable(QualityHeatMapsPerTileAndBase qualityHeatMapsPerTileAndBase, ResultsTableModel model) {
		super(model);
		...
		setDefaultRenderer(MeanQualityMatrix.class, new MatrixCellRenderer());
		setTableHeader(null);
	...
	public void updateModel(LaneCoordinates laneCoordinates) {			
		ResultsTableModel model = new ResultsTableModel(this.qualityHeatMapsPerTileAndBase, laneCoordinates, getModel().getTileNumeration());			
		setModel(model);		
	...
	protected JDialog createPopupHeatMapWindow(int row, int col) {
		JDialog result = null;
			result = new InteractiveHeatmapDialog(row, col, this);
//			result.setLocation(400, 350);
//			result.setSize(500, 500 * rowSpan / colSpan);
